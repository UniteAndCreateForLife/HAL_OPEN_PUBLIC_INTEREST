...
def test_provenance_verified_corpus_is_scored(tmp_path):
    ...
    assert (
        report["failure_analysis"]["per_expected_status"]["accept"]["recall"]
        == 1.0
    )
...
def test_failure_analysis_records_mismatch_without_diagnostic_inference(
    tmp_path,
):
    ...
    assert (
        analysis["per_expected_status"]["human_review"]["recall"]
        == 0.0
    )
...
def test_hash_mismatch_fails_closed(tmp_path):
    with pytest.raises(ValueError, match="SHA256 mismatch"):
        evaluate_corpus(
            _fixture(tmp_path, digest_override="0" * 64),
            tmp_path,
        )
...
def test_agentic_first_action_and_enhancement_expectations_are_scored(
    tmp_path,
):
    ...
    manifest = {
        ...
        "items": [{
            ...
            "source_page_url": "https://bbbc.broadinstitute.org/BBBC038",
            "license": "CC0-1.0",
            "license_url": "https://creativecommons.org/publicdomain/zero/1.0/",
            "attribution": "BBBC038v1 contributors",
            "derivation": "controlled_illumination_gradient",
            "expected_qc_status": "accept",
            "expected_first_action": "enhance_and_reanalyze",
            "expected_enhancement": True,
        }],
    }
    manifest_path = tmp_path / "manifest-agentic.json"
    manifest_path.write_text(json.dumps(manifest), encoding="utf-8")
    ...
    assert (
        report["items"][0]["actual_first_action"]
...
def test_agentic_action_mismatch_is_visible(tmp_path):
    path = _fixture(tmp_path)
    data = json.loads(path.read_text())
    data["items"][0]["expected_qc_status"] = None
    data["items"][0]["expected_first_action"] = "request_recapture_focus"
    path.write_text(json.dumps(data))
    report = evaluate_corpus(path, tmp_path)
    assert report["first_action_agreement"] == 0.0
    assert report["combined_expectation_agreement"] == 0.0
    assert report["failure_analysis"]["first_action"]["failure_ids"] == [
        "sample-001"
    ]
...
def test_extended_provenance_fields_are_validated(tmp_path):
    ...
    data = json.loads(path.read_text())
    item = data["items"][0]
    item["source_page_url"] = "https://bbbc.broadinstitute.org/BBBC038"
    item["license_url"] = "https://creativecommons.org/publicdomain/zero/1.0/"
    item["parent_sha256"] = "0" * 64
    item["derivation"] = "native"
    path.write_text(json.dumps(data))
