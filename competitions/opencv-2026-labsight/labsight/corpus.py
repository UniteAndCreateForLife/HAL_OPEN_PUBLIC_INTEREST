...
def _validate_sha256(value: str, *, field: str, item_id: str) -> None:
    if len(value) != 64 or any(c not in "0123456789abcdefABCDEF" for c in value):
        raise ValueError(f"{item_id}: {field} must be a 64-character hexadecimal digest")
...
def load_manifest(path: Path) -> list[CorpusItem]:
    ...
    for item in items:
        ...
        if item.source_page_url is not None and not _is_http_url(item.source_page_url):
        ...
        if item.parent_sha256 is not None:
            _validate_sha256(item.parent_sha256, field="parent_sha256", item_id=item.id)
        if item.derivation is not None and not item.derivation.strip():
            raise ValueError(f"{item.id}: derivation must be non-empty when provided")
        ...
        if item.expected_first_action is not None and item.expected_first_action not in ALLOWED_AGENT_ACTIONS:
            raise ValueError(f"{item.id}: unsupported expected_first_action {item.expected_first_action!r}")
    return items
...
def _evaluation_id(items: list[CorpusItem]) -> str:
    ...
    contract = [{
        ...
        "parent_sha256": None if item.parent_sha256 is None else item.parent_sha256.lower(),
...
def _categorical_failure_analysis(rows: list[dict], expected_key: str, actual_key: str, match_key: str) -> dict:
    ...
    for expected in sorted({str(row[expected_key]) for row in scored}):
        group = [row for row in scored if str(row[expected_key]) == expected]
        matched = sum(bool(row[match_key]) for row in group)
        per_expected[expected] = {"samples": len(group), "correct": matched, "recall": matched / len(group)}
    return {
...
def _safety_failure_analysis(rows: list[dict]) -> dict:
    ...
    return {
        ...
        "unsafe_accept_ids": [str(row["id"]) for row in unsafe_accepts],
...
def _failure_analysis(rows: list[dict]) -> dict:
    final_status = _categorical_failure_analysis(rows, "expected_qc_status", "actual_qc_status", "matches_expected")
    first_action = _categorical_failure_analysis(rows, "expected_first_action", "actual_first_action", "first_action_matches")
    enhancement = _categorical_failure_analysis(rows, "expected_enhancement", "used_enhancement", "enhancement_matches")
    return {**final_status, "safety": _safety_failure_analysis(rows), "first_action": first_action, "enhancement": enhancement}
...
def _combined_expectation_match(row: dict) -> bool | None:
    values = [row["matches_expected"], row["first_action_matches"], row["enhancement_matches"]]
...
def evaluate_corpus(manifest_path: Path, image_root: Path) -> dict:
    ...
    for item in items:
        ...
        first_match = None if item.expected_first_action is None else actual_first_action == item.expected_first_action
        enhancement_match = None if item.expected_enhancement is None else result.used_enhancement == item.expected_enhancement
        row = {
            "id": item.id,
        ...
        row["combined_expectation_match"] = _combined_expectation_match(row)
        rows.append(row)
    ...
    return {
        ...
        "samples": len(rows),
        "scored_samples": sum(row["matches_expected"] is not None for row in rows),
        "qc_agreement": _agreement(rows, "matches_expected"),
        "first_action_agreement": _agreement(rows, "first_action_matches"),
        "enhancement_agreement": _agreement(rows, "enhancement_matches"),
        "combined_expectation_agreement": _agreement(rows, "combined_expectation_match"),
        "latency_ms": {
